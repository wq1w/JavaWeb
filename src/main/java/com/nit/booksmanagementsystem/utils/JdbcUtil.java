...
public class JdbcUtil {
     ...
     * @param params 要填充到 SQL 语句中的参数。如果没有参数,则传入一个空的可变长参数数组。
     * @return 查询结果集。
     * @throws RuntimeException 如果输入的 SQL 语句为空或不是 SELECT 语句,会抛出此异常。
     */
    public static ResultSet query(String SQL, Object... params) {
        // 检查 SQL 语句是否为空或非 SELECT 语句
        if (SQL == null || SQL.trim().isEmpty() || !SQL.trim().toLowerCase().startsWith("select")) {
            throw new RuntimeException("你的SQL语句为空或不是查询语句");
        }
        ResultSet rs = null;
        if (params.length > 0) {
            // 如果有参数,则使用 PreparedStatement 执行查询
